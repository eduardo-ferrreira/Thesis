...
g = 9.81 # m s ^ -2
p_water = 1000 # kg m ^ -3
r_rod = 0.004 # m radius of the hole inside the rod
l_rod = 0.629 # m
A_rod = 0.035*0.004*2+np.pi*(10.5e-3-6.5e-3)**2-np.pi*r_rod**2 
...
def simulation(source, criticality, stoptime, limit):
    ...
    while 10 < initial_values[0] < 10**15 and time.time()-t4 < stoptime:
        ...
        while k_value < 1 and cont == True:
            ...
            print(f'k: {k_value}, Counts: {initial_values[0]:.3e}, Cont: {cont}, Current: {current:.3e}, Raspberry output: {raspberry_output}, Scram: {scram}, Time passed: {(time.time()-t4)/60}')
            ...
            #print(time.time()-t_init_cycle)
            new_k, scram, state_vector = k(v_values=carapau, criticality=criticality, rasp_output=raspberry_output, scram=scram, state_vector=state_vector) #new k calculated
