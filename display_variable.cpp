...
int main() {
	...
	std::string filename = "elapsed_time.csv";
    	int row = 1;  // For example, get the value from the third row (index 2)
    	int col = 1;  // For example, get the value from the second column (index 1)
    	...
    	std::string value_str = std::to_string(value); // Convert the variable to a string to display on the LCD

        std::cout << "Time: " << value << std::endl;
        lcd_display("Time: " + value_str);
    }
