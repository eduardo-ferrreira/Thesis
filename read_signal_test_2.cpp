...
int main() {
    ...
    // Set GPIO 17 as output and GPIO 18 as input with pull-down resistor
    pinMode(17, OUTPUT);
    pinMode(18, INPUT);
	...
	std::cout << "GPIO 17: HIGH, GPIO 18 state: " << state << std::endl;
        lcd_display("GPIO 17: HIGH");

        digitalWrite(17, LOW);
        sleep(5);
        state = digitalRead(18);
	std::cout << "GPIO 17: LOW, GPIO 18 state: " << state << std::endl;
        lcd_display("GPIO 17: LOW");
    }
